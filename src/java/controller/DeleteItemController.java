...
public class DeleteItemController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            boolean ok = dao.deleteItemByIdAndUser(itemId, userId);

            if (ok) {
                session.setAttribute("successMessage", "Đã xóa bài đăng thành công.");
            } else {
                session.setAttribute("successMessage", "Không xóa được (có thể không phải bài của bạn).");
            }

        } catch (Exception e) {
            session.setAttribute("successMessage", "Lỗi xóa bài: " + e.getMessage());
        }
